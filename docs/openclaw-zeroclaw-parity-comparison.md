...
|---|---|---|---|
| Primary install path | `cargo` workspace setup with one-command local bootstrap (`scripts/bootstrap_local.sh`) and optional `--clone-adk <url>` dependency bootstrap, plus local binary installer (`scripts/install_local_release.sh`) | install script or npm/pnpm global install (`openclaw`) | Homebrew/bootstrap/cargo install paths |
...
|---|---|---|---|---|---|
| 1 | Install from scratch on macOS/Linux | Partial | Full | Full | LiveClaw now has one-command bootstrap, optional adk-rust auto-clone, and local release installer, but still remains source-centric |
...
| 17 | Integrate Telegram/Slack/Webhook ingress | Full | Full | Full | LiveClaw implements adapter and route tests for Telegram/Slack/Webhook plus Discord, Matrix, and Teams |
...
2. Channel breadth gap: LiveClaw now covers Telegram/Slack/Webhook/Discord/Matrix/Teams, but still trails broader ecosystem depth.
