...
|---|---|---|---|
| Primary install path | `cargo` workspace setup with one-command local bootstrap (`scripts/bootstrap_local.sh`) and optional `--clone-adk <url>` dependency bootstrap | install script or npm/pnpm global install (`openclaw`) | Homebrew/bootstrap/cargo install paths |
...
|---|---|---|---|---|---|
| 1 | Install from scratch on macOS/Linux | Partial | Full | Full | LiveClaw now has one-command bootstrap and optional adk-rust auto-clone, but still remains source-centric |
